var app = {
	...
	initialize: function() {
		...
		$( ".tabs" ).click(function() {
			...
			($(this).data("tab")==1) ? app.tab_1() :
			($(this).data("tab")==2) ? app.tab_2() :
			($(this).data("tab")==3) ? app.tab_3() :
			($(this).data("tab")==4) ? app.tab_4() :
			($(this).data("tab")==5) ? app.tab_5() : 
			($(this).data("tab")==6) ? app.tab_6() :
			($(this).data("tab")==7) ? app.tab_7() : console.log("Other") ;
	...
	sampler: function(type) {
		...
		(app.tag==5) ? app.tab_5() : 
		(app.tag==6) ? app.tab_6() :
		(app.tag==7) ? app.tab_7() : console.log("Other") ;
	...
	tab_1: function() {
		...
		let html = '<table id="tab1" class="councilTable">';
        html += '<thead><tr>';
	...
	tab_2: function() {
        ...
        let html =  '<table id="tab2" class="councilTable">';
		html += '<thead>';
		html += '<tr>';
		html += '<th class="hidden">Id</th>';
		html += '<th>Type<img src="images/info.png"></th>';
	...
	tab_3: function() {
        ...
        let html = '<table id="tab3" class="councilTable">';
		html += '<thead>';
		html += '<tr>';
		html += '<th class="hidden">Id</th>';
		html += '<th>Type<img src="images/info.png"></th>';
	...
	tab_4: function() {
        ...
        let html = '<table id="tab4" class="councilTable">';
		html += '<thead>';
		html += '<tr>';
		html += '<th class="hidden">Id</th>';
		html += '<th>Type<img src="images/info.png"></th>';
	...
	tab_5: function() {
        ...
        let html = '<table id="tab5" class="councilTable">';
		html += '<thead>';
		html += '<tr>';
		html += '<th class="hidden">Id</th>';
		html += '<th>Type<img src="images/info.png"></th>';
	...
	tab_6: function() {
        ...
        let html = '<table id="tab6" class="councilTable">';
		html += '<thead>';
		html += '<tr>';
		html += '<th class="hidden">Id</th>';
		html += '<th>Type<img src="images/info.png"></th>';
	...
	tab_7: function() {
        ...
        let html = '<table id="tab7" class="councilTable">';
		html += '<thead>';
		html += '<tr>';
		html += '<th class="hidden">Id</th>';
		html += '<th>Type<img src="images/info.png"></th>';
	...
	generate: function() {
		...
		let local = app.council[0].b;

		let html = '';

		let varchar = factsheet[app.x].target

		// Council calculations

		let council = parseFloat(app.council[0][varchar].replace(/[^\d\.\-eE+]/g, ""));

		let label = (isNaN(council)) ? 'not supplied' : '$' + app.makeItLookNice(council);

		// Group calculations

		app.group.forEach(function(value, index) {

			let num = parseFloat(value[varchar].replace(/[^\d\.\-eE+]/g, ""));

			(!isNaN(num)) ? group.push(num) : '' ;

		});

		let group_total = group.reduce((a, b) => a + b, 0) // Total

		let group_average = parseInt(group_total / group.length) // Avergae of group
		...
		let national_total = national.reduce((a, b) => a + b, 0) // Total

		let national_average = parseInt(national_total / national.length) // National average

		let max = Math.max.apply(null, national); // Maximum value
	...
	proportion: function() {
		...
		let council = Math.round((100 / parseFloat(app.council[0].f.replace(/[^\d\.\-eE+]/g, ""))) * parseFloat(app.council[0].a1.replace(/[^\d\.\-eE+]/g, "")))
		...
		app.group.forEach(function(value, index) {
			...
			let num = Math.round((100 / parseFloat(value.f.replace(/[^\d\.\-eE+]/g, ""))) * parseFloat(value.a1.replace(/[^\d\.\-eE+]/g, "")));
	...
	tagger: function(type) {

		return (type=='P') ? 'provincial councils' :
		(type=='U') ? 'unitary councils' : 
		(type=='M') ? 'metro councils' : 
		(type=='R') ? 'rural councils' : 'city councils' ;

	},

	formal: function(type) {

		return (type=='P') ? 'Provincial' :
		(type=='U') ? 'Unitary' : 
		(type=='M') ? 'Metro' : 
		(type=='R') ? 'Rural' : 'City' ;
